...
-- CACH 1
SELECT TOP 1 WITH TIES H.MAHV, H.HO, H.TEN, L.MALOP, COUNT(*) AS SoMonDat
FROM KETQUATHI KQ
JOIN HOCVIEN H ON KQ.MAHV = H.MAHV
JOIN LOP L ON H.MALOP = L.MALOP
WHERE KQ.DIEM IN (9, 10)
GROUP BY H.MAHV, H.HO, H.TEN, L.MALOP
ORDER BY SoMonDat DESC


-- CACH 2
WITH RANKED AS(
	SELECT HV.MAHV, HV.HO +' '+ HV.TEN HOTEN, COUNT(KQT.MAMH) SOMON,
		ROW_NUMBER() OVER (PARTITION BY HV.MALOP ORDER BY COUNT(KQT.MAMH) DESC) RANKI
	FROM HOCVIEN HV, KETQUATHI KQT
	WHERE HV.MAHV = KQT.MAHV
	AND KQT.DIEM IN (9,10)
	GROUP BY HV.MALOP, HV.MAHV, HV.HO, HV.TEN
)
SELECT TOP 1 R.MAHV, R.MAHV, R.HOTEN, R.SOMON
FROM RANKED R
WHERE RANKI = 1
-- CACH 3
-- Bước 1: Tính số môn đạt điểm 9, 10 của từng học viên trong từng lớp
WITH SoMonDat AS (
    SELECT HV.MALOP, HV.MAHV, HV.HO, HV.TEN, COUNT(KQ.MAMH) AS SoMon
    FROM HOCVIEN HV
    JOIN KETQUATHI KQ ON HV.MAHV = KQ.MAHV
    WHERE KQ.DIEM IN (9, 10)
    GROUP BY HV.MALOP, HV.MAHV, HV.HO, HV.TEN
),
-- Bước 2: Tìm số môn lớn nhất trong từng lớp
MaxSoMonDat AS (
    SELECT MALOP, MAX(SoMon) AS MaxSoMon
    FROM SoMonDat
    GROUP BY MALOP
)
-- Bước 3: Kết hợp để lấy học viên có số môn đạt lớn nhất trong từng lớp
SELECT TOP 1 S.MALOP, S.MAHV, S.HO, S.TEN, S.SoMon
FROM SoMonDat S
JOIN MaxSoMonDat M ON S.MALOP = M.MALOP AND S.SoMon = M.MaxSoMon;
...
WITH
DAYHOC AS(
	SELECT HOCKY, NAM, MAGV, COUNT(*) AS SOTIET
	FROM GIANGDAY
	GROUP BY HOCKY, NAM, MAGV
)
SELECT TOP 1 WITH TIES HOCKY, NAM, GV.MAGV, GV.MAGV, GV.HOTEN, SOTIET
FROM DAYHOC, GIAOVIEN GV
WHERE GV.MAGV = DAYHOC.MAGV
ORDER BY SOTIET DESC

-- CACH 2
SELECT TOP 1 WITH TIES GD.MAGV, G.HOTEN, COUNT(*) AS SoMon
FROM GIANGDAY GD
JOIN GIAOVIEN G ON GD.MAGV = G.MAGV
GROUP BY GD.MAGV, G.HOTEN, GD.HOCKY, GD.NAM
ORDER BY SoMon DESC
-- CACH 3
...
SELECT HOCKY, NAM, GV.MAGV, GV.MAGV, GV.HOTEN, SOTIET 
FROM DAYHOC, GIAOVIEN GV
WHERE GV.MAGV = DAYHOC.MAGV AND SOTIET = (SELECT MAX(SOTIET) FROM DAYHOC)
